...
import net.voidkin.voidkin.Config;
import net.voidkin.voidkin.block.ModBlocks;
import net.voidkin.voidkin.util.LandmarkUtil;
import net.voidkin.voidkin.util.ModTags;
import net.voidkin.voidkin.util.Restriction;
import net.voidkin.voidkin.world.StructureTagGenerator;
...
public class VoidTeleporter {
    ...
    protected static DimensionTransition createPosition(ServerLevel dest, Entity entity, BlockPos destPos, TeleporterCache cache, boolean locked) {
        ...
        BlockPos portalPos = makePortal(cache, entity, dest, entity.position(), locked);
    ...
    protected static DimensionTransition placeInExistingPortal(TeleporterCache cache, ServerLevel destDim, Entity entity, BlockPos pos) {
        ...
        if (portalPosition != null) {
            ...
            if (blockpos == null || !destDim.getBlockState(blockpos).is(ModBlocks.XION_PORTAL)) {
        ...
        if (blockpos == null) {
            ...
            if (flag) {
                ...
                XionPortalBlock.recursivelyValidatePortal(entity.level(), entity.blockPosition(), portalBlocks, new MutableInt(0), entity.level().getBlockState(entity.blockPosition()));
            ...
            return makeTransition(destDim, entity, entity.position() /*safePosInColumn(destDim, entity, portalX, portalY, portalZ)*/);
        }
    }
    ...
    private static boolean isPortal(BlockState state) {
        return state.is(ModBlocks.XION_PORTAL);
    }
    ...
    protected static double getHorizontalScale(ServerLevel destination) {
        ...
        return destination.dimension().equals(ModDimensions.DEATH_DIM_LEVEL_KEY) ? 1F / scale : scale;
    }
    ...
    protected static Vec3 moveToSafeCoords(ServerLevel level, Entity entity, BlockPos pos) {
        ...
        if (isSafeAround(level, pos, entity, checkProgression)) {
            ...
            return entity.position();
                    //safePosInColumn(level, entity, Vec3.atCenterOf(pos));
        }
    ...
    private static boolean isUnsafe(Level world, BlockPos pos, Entity entity, boolean checkProgression) {
        if (!world.dimension().equals(ModDimensions.DEATH_DIM_LEVEL_KEY)) {
            return false;
        }
    ...
    private static void cacheNewPortalCoords(TeleporterCache cache, @Nullable ServerLevel srcDim, BlockPos pos, BlockPos srcPos) {
        ...
        cache.addBlockToCache(srcDim.dimension().location(), new ColumnPos(pos.getX(), pos.getZ()), new VoidTeleporter.PortalPosition(exitPos, srcDim.getGameTime()));
        cache.addBlockToCache(srcDim.dimension().location(), new ColumnPos(pos.south().getX(), pos.south().getZ()), new VoidTeleporter.PortalPosition(exitPos, srcDim.getGameTime()));
        cache.addBlockToCache(srcDim.dimension().location(), new ColumnPos(pos.east().getX(), pos.east().getZ()), new VoidTeleporter.PortalPosition(exitPos, srcDim.getGameTime()));
        cache.addBlockToCache(srcDim.dimension().location(), new ColumnPos(pos.south().east().getX(), pos.south().east().getZ()), new VoidTeleporter.PortalPosition(exitPos, srcDim.getGameTime()));
    }
    ...
    protected static BlockPos makePortalAt(Level world, BlockPos pos, boolean locked) {
        // grass all around it
        BlockState grass = ModBlocks.BLOOD_WOOD.get().defaultBlockState();
        ...
        // dirt under it
        BlockState dirt = ModBlocks.BLOOD_WOOD.get().defaultBlockState();
        for (BlockPos blockpos : positions) {
            BlockState state = world.getBlockState(blockpos);
            if (state.is(ModBlocks.BLOOD_WOOD.get()) || state.is(BlockTags.REPLACEABLE) || state.is(BlockTags.AIR))
                world.setBlockAndUpdate(blockpos, dirt);
        }

        // portal in it
        BlockState portal = ModBlocks.XION_PORTAL.get().defaultBlockState().setValue(XionPortalBlock.DISALLOW_RETURN, Config.shouldReturnPortalBeUsable);
    ...
    private static BlockState randNatureBlock(RandomSource random) {
        Optional<Block> optional = BuiltInRegistries.BLOCK
                .getTag(ModTags.Blocks.VOIDKIN_BLOCKS)
