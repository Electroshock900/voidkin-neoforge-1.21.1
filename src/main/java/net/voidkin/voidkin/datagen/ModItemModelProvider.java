...
public class ModItemModelProvider extends ItemModelProvider {
    ...
    public ModItemModelProvider(PackOutput output,  ExistingFileHelper existingFileHelper) {
        super(output, Voidkin.MODID,existingFileHelper);
    }
    ...
    private void trimmedArmorItem(DeferredItem<Item> itemRegistryObject) {
        final String MOD_ID = Voidkin.MODID; // Change this to your mod id
    ...
    private ItemModelBuilder saplingItem(DeferredBlock<Block> item) {
        return withExistingParent(item.getId().getPath(),
                ...
                ResourceLocation.fromNamespaceAndPath(Voidkin.MODID,"block/" + item.getId().getPath()));
    }

    private ItemModelBuilder simpleItem(DeferredItem<Item> item) {
        return withExistingParent(item.getId().getPath(),
                ResourceLocation.withDefaultNamespace("item/generated")).texture("layer0",
                ResourceLocation.fromNamespaceAndPath(Voidkin.MODID,"item/" + item.getId().getPath()));
    }


    public void buttonItem(DeferredBlock<?> block, DeferredBlock<Block> baseBlock) {
        this.withExistingParent(block.getId().getPath(), mcLoc("block/button_inventory"))
                .texture("texture",  ResourceLocation.fromNamespaceAndPath(Voidkin.MOD_ID,
    ...
    public void wallItem(DeferredBlock<?> block, DeferredBlock<Block> baseBlock) {
        this.withExistingParent(block.getId().getPath(), mcLoc("block/wall_inventory"))
                .texture("wall",  ResourceLocation.fromNamespaceAndPath(Voidkin.MOD_ID,
                        "block/" + baseBlock.getId().getPath()));
    }
    ...
    private ItemModelBuilder handheldItem(DeferredItem<?> item) {
        return withExistingParent(item.getId().getPath(),
                ...
                ResourceLocation.fromNamespaceAndPath(Voidkin.MOD_ID,"item/" + item.getId().getPath()));
    }
